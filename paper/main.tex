...
\author{
  Meftun Akarsu\textsuperscript{1} \quad
  Christopher Mierbach\textsuperscript{2} \quad
  Recep Kaan Karaman\textsuperscript{3} \\[6pt]
  \textsuperscript{1}Technische Hochschule Ingolstadt \quad
  \textsuperscript{2}Radiate \quad
  \textsuperscript{3}Uludağ Üniversitesi \\[2pt]
  \texttt{mea5963@thi.de} \quad
  \texttt{christopher@radiate.com} \quad
...
\paragraph{HyDE.}
HyDE \citep{gao2022hyde} addresses the asymmetry between short queries and long documents by generating a hypothetical answer passage at query time and retrieving with its embedding rather than the original query embedding. The generated document may contain hallucinations, but the dense encoder grounds it to the actual corpus by mapping it into the same embedding space as real documents \citep{gao2022hyde}. We prompt GPT-4.1-mini at temperature 0 to generate a plausible answer for each query, embed it with \texttt{text-embedding-3-large}, and retrieve against the corpus index. HyDE was originally shown to outperform unsupervised dense retrievers on web search, QA, and fact verification tasks \citep{gao2022hyde}; its behaviour on numerical financial QA is one of the questions this paper investigates.
...
\midrule
Adaptive
  & CRAG (gpt-4.1-mini)         & 0.302 & 0.556 & 0.658 & 0.788 & 0.415 & 0.536 & 0.456 \\
\midrule
\multirow{2}{*}{Fusion}
...
\midrule
BM25          & 0.251 & \textsuperscript{$\dagger$} \\
Dense         & 0.257 & \textsuperscript{$\dagger$} \\
Hybrid RRF    & 0.282 & 0.346 \\
Oracle        & 0.350 & 0.403 \\
\bottomrule
\end{tabular}
\caption{End-to-end Number Match (NM) by retrieval method and LLM. Better retrieval consistently leads to better generation quality. GPT-5.4 outperforms GPT-4.1-mini by 6--7pp on the same retrieval. \textsuperscript{$\dagger$}Not evaluated.}
...
Table~\ref{tab:full_results} presents the complete set of retrieval metrics for all evaluated methods. Values are taken from the main experiments (Table~\ref{tab:main_results}).
...
\bottomrule
\end{tabular}
\caption{Full retrieval results for all methods and metrics on T\textsuperscript{2}-RAGBench (23,088 queries, 7,318 documents). Methods are sorted by nDCG@10 in ascending order. Best results in \textbf{bold}. \textsuperscript{$\ast$}Hybrid + Rerank returns at most 10 documents, so R@20 is not applicable.}
...
the document's relevance to answering the
question.
